...
@bp.route('/')
@login_required #funcion decoradora que se encuentra dentro de auth.py, se encarga de proteger la funcion index
def index():
    db, c = get_db()
    c.execute(
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'select t.id, t.description, t.completed, t.create_by, created_at, u.username from todo t join user u on t.create_by = u.id where t.id = %s',
        (id,)
    )
